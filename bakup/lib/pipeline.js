...
async function runProvider(provider, opts = {}) {
  ...
  // 6. Cache lookup (single round trip)
  const cached = await lookupCachedClassifications(
    provider.data_source,
    deduped.map(e => e.body_hash)
  );

  // 7. LLM for cache misses
  ...
  for (const e of deduped) {
    ...
    if (llm && llm.is_closure !== e.regex.is_closure) {
      audit_events.push({
        kind: 'disagreement',
  ...
  const closure_entries = deduped
    .filter((e) => isCurrentlyClosed(e.verdict) && e.hospital_id)
    .map((e) => ({
      hospital_id:     e.hospital_id,
      body_hash:       e.body_hash,
      reopen_phrase:   e.verdict.reopen_phrase,
      expected_reopen: e.verdict.expected_reopen,
