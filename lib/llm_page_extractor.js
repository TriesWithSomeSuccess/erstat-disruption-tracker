...
function buildExtractionPrompt(opts) {
  const { province, today, source_label } = opts;
...
- Province: ${province}
- Source: ${source_label}
- Today's date (provincial local time): ${today}
...
ERstat tracks Emergency Department status only. If the article describes a disruption to a service other than the Emergency Department (obstetrics, laboratory, diagnostic imaging, pharmacy, etc.)
 and the ED itself remains open, set is_closure: false. Only set is_closure: true when the ED is affected. either fully closed, on diversion, on reduced hours, or operating with virtual physician coverage.
...

{
  ...
  "body_excerpt":      "<the exact sentences from the page describing this entry, max 600 chars>",
...
2. If a date range is given in local provincial time (e.g. "8 p.m. Saturday, May 17 until 8 a.m. Monday, May 19"), express closed_from and closed_until as ISO 8601 with the correct timezone offset for the province. Atlantic Time (NS/NB/PE/NL): -03:00 during DST. Eastern (ON/QC): -04:00 during DST. Central (MB/SK): -06:00. Mountain (AB): -06:00. Pacific (BC): -07:00. Today is ${today}; infer the year from context if a year isn't stated.
...
5. For non-ED disruptions (e.g. obstetrics, lab, physio), still emit the entry — is_closure may still be true if the SERVICE is closed/unavailable. The pipeline decides downstream what to do with non-ED closures.

6. If the page contains a cross-facility service notice (e.g. "Physiotherapy shortage in Moncton, Saint John, and Miramichi"), emit ONE entry per named area with facility_name = the area name as-given. Mark scope as "reduced" if appointments are merely affected, "partial_service" if some appointments are cancelled.

7. If the page genuinely lists no current closures (e.g. an empty "Current closures" section, or a "no current notices" placeholder), return an empty array: [].

8. Output ONLY the JSON array. No prose, no markdown fences, no commentary. The response must begin with [ and end with ].
...
async function extractFromPage(opts) {
  ...
  const today = new Intl.DateTimeFormat('en-CA', {
    timeZone: opts.timezone || 'America/Halifax',
...
function buildArticlePrompt(opts) {
  const { content, title, url, province, today, source_label } = opts;
...

{
  ...
  "body_excerpt":      "<key sentences from the article, max 800 chars>",
  "confidence":        "high" | "medium" | "low",
...
5. Times: Atlantic Time (-03:00 in DST, -04:00 in standard). Today is ${today}. Express closed_from / closed_until as ISO 8601 with the correct offset. If the article gives only a date with no time, use the most reasonable boundary (closure starts at 00:00, closure ends at 23:59 of stated day) and note that in reasoning.

6. Output ONLY the JSON object. No prose, no markdown fences. Begin with { and end with }.
...
function makeLlmListProvider(config) {
  ...
  async function discoverAndExtract() {
    ...
    //
    // The cache stores the LLM extraction result keyed by URL. On hit, we
    // rebuild the ParsedEntry from the cached LLM output. On miss, we fetch
    // and call Haiku, then write the result to cache.
    //
    ...
    for (const item of items) {
      const cacheKey = hash(dataSource, 'list_article_v1', item.url);
      let result;
      ...
      if (cached && cached.llm_output) {
        ...
        try {
          ...
          const content = extractVisibleContent(articleHtml);
          ...
          new_items++;
          // Cache for next pass — store the LLM output verbatim so we can
          // reconstruct the same ParsedEntry deterministically.
          try {
            await articleCache.set(cacheKey, { url: item.url, llm_output: result });
          } catch (err) {
